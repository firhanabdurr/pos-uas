  ...
  const handlerSubmit = async (value) => {
    ...
    try {
      const newObject = {
        ...
        cartItems,
        subTotal,
        tax: Number(((subTotal / 100) * 10).toFixed(3)),
        totalAmount: Number((Number(subTotal) + Number(((subTotal / 100) * 10).toFixed(3))).toFixed(2)),
  ...
  return (
    <Layout>
      ...
      <div className="subTotal">
        <h2>Sub Total: <span>Rp {(subTotal).toFixed(3)}</span></h2>
        ...
        <Form layout='vertical' onFinish={handlerSubmit}>
          ...
          <Form.Item name="paymentMethod" label="Payment Method">
            <Select>
              ...
              <Select.Option value="Dana">Dana digital</Select.Option>
              <Select.Option value="Card">Card</Select.Option>
            </Select>
          </Form.Item>
          <div className="total">
            <span>SubTotal: Rp{(subTotal.toFixed(3))}</span><br />
            <span>Tax: Rp{((subTotal / 200) * 10).toFixed(3)}</span>
            <h3>Total: Rp{(Number(subTotal) + Number(((subTotal / 200) * 10).toFixed(3))).toFixed(3)}</h3>
          </div>
